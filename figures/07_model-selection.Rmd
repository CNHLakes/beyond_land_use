...
fit0 <- get_if_not_exists(brm_fit, "fit0.rds", formula = model_form, data = dt)
print("fixed effect")
print(fit0$formula)

# linear re
model_form <- bf(tp ~ (1 + agvpct | hu4vzoneid))
fit <- get_if_not_exists(brm_fit, "fit.rds", formula = model_form, data = dt)
# plot(fit)
# print(fit$formula)
...
model_form <- bf(tp ~ bagvpct * agvpct, 
                 bagvpct ~ 1 + 
                   (1|hu4vzoneid), 
                 nl = TRUE)
fit1 <- get_if_not_exists(brm_fit, "fit1.rds", formula = model_form, data = dt)
# plot(fit1)
...
model_form2 <- bf(tp ~ bagvpct * agvpct, 
                 bagvpct ~ 1 + phosphorusvfertilizervuse + soybeans +
                   (1|hu4vzoneid), 
                 nl = TRUE)
fit2 <- get_if_not_exists(brm_fit, "fit2.rds", formula = model_form2, data = dt)
