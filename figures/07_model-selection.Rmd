...
```{r 07_model_selection, fig.height=3, echo=FALSE,eval=TRUE}
...
signif_hu4_ids <- re_brms[[5]]$re_signif_ids # 5 is best tn model with re effects
...
plot_grid(
  ...
  gplot_grid(gg_hu4_n, gg_hu4_ndep, ncol = 1),
  rel_heights = c(0.6, 1), labels = "AUTO", ncol = 2
)
``` 

```{r residual_dotplot, echo=FALSE}
...
```{r model_fitting, echo=FALSE, eval=TRUE}
library(brms)
library(tidybayes)
library(modelr)
...
```{r model_diagnostics_advanced, fig.height=4, echo=FALSE, eval=TRUE}
...
```{r signif_map, echo=FALSE, eval=TRUE}
ggplot() + 
  geom_sf(data = hu4) +
