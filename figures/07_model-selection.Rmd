...
# re_brms[[6]]$formula
best_tp        <- 6 # 6 is best tp model with random effects
...
gg_tp_fe <- 
  ...
  # x-axis labels
  scale_y_continuous(breaks = seq(from = -0.6, to  = 0.6, by = 0.2), 
  ...
  theme(panel.background = element_rect(fill = "transparent", color = NA),
    ...
    annotate(x=0, xend=0, y=-0.6, yend=0.6, 
...
# lapply(re_brms, function(x) x$formula) 
# re_brms[[11]]$formula
best_tn        <- 11 # 11 is best tn model with random effects
...
hu4_all       <- st_read("../data/gis.gpkg", "hu4s_focal", quiet = TRUE)
...
cowplot::plot_grid(
  ...
  rel_widths = c(1, 0.95)), 
ggplot(data.frame(x = 1, y = 1, label = "Standardized slopes", stringsAsFactors = FALSE)) + 
