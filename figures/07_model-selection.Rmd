...
text_colors <- rev(ifelse(gg_tp$signif == 0, "black", "red"))
# cbind(text_colors, as.character(gg_tp$pretty))

gg_tp_fe <- ggplot(data = gg_tp) + 
  geom_pointrange(aes(x = pretty, y = `50%`, 
                      ymin = `5%`, ymax = `95%`, color = signif)) + 
  expand_limits(y = -1.2) +
  geom_rect(ymin = -0.6, ymax = -1.5, xmin = 0, xmax = nrow(gg_tp) + 1, 
              fill = "white") +
  geom_text(aes(x = pretty, label = pretty, color = signif), 
            y = -0.65, vjust = 1.1, hjust = "right") + 
...
gg_tn <- spread_draws(fit_tn, `b_.*`, regex = TRUE) %>%
  ...
  group_by(pretty) %>%
  summarize(value = list(tibble::enframe(
    quantile(value, probs=c(0.05,0.5,0.95))))) %>% 
  unnest() %>%
  tidyr::spread(name, value) %>%
  rowwise() %>%
  mutate(signif = (`5%` > 0 & `95%` > 0)| (`5%` < 0 & `95%` < 0))
...
gg_tn_fe <- ggplot(data = gg_tn) + 
  ...
  theme_cowplot() +
  ylim(-0.6, 0.4) +
  ylab("Standardized FE slope") + theme(legend.position = 0) + ggtitle("TN ~ Slope") +
  xlab("") 
```
