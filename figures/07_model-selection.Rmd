...
```{r tp_summary, fig.width=4, fig.height=4.25}
...
gg_tp_re <- ggplot(data = fit_tp$re) +
  ...
  ylab("TP model, pasture RE slope") + xlab("Zone ID") +
  theme(legend.position = 0)

gg_tp_re
```

```{r tn_summary, fig.height=4, fig.width=4.25}
# lapply(re_brms, function(x) x$formula) 
...
gg_tn_re <- ggplot(data = fit_tn$re) +
  ...
  cowplot::theme_cowplot() +
  theme(rect = element_rect(fill = "transparent")) +
  ylab("TN model, corn RE slope") + xlab("Zone ID") + 
  theme(legend.position = 0)

gg_tn_re
```

```{r tn_re_compare, fig.height=3}
