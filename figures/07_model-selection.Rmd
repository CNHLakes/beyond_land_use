...
dt        <- readRDS("../data/dt.rds") %>% 
  dplyr::select("lagoslakeid", "hu4_zoneid", "hu12_zoneid", 
                "tp", "hu12_ppt_mean", "hu12_baseflow_mean",
...
(model_forms <- list(
  "ag" = bf(tp ~ ag), 
  "depth+bf" = bf(tp ~ ag + maxdepth + hu12vbaseflowvmean),
  "depth+iwsla" = bf(tp ~ ag + maxdepth + iwslavratio), 
  "all" = bf(tp ~ ag + maxdepth + hu12vbaseflowvmean + iwslavratio) 
))
...
dplyr::bind_rows(
  ...
  dplyr::select(Response, Model, Estimate) 
