...
```{r tp_fe, out.width="3.25in", out.height="4in"}
# lapply(re_brms, function(x) x$formula) 
fit_tp   <- re_brms[[1]] # 1 is best tp model with random effects
...
gg_tp <- spread_draws(fit_tp, `b_.*`, regex = TRUE) %>%
  ...
  mutate(category = factor(category, 
                           levels = rev(c("Nutrient inputs", "Nutrient transport", 
                                      "Land-use cover", "Buffer composition", 
                                      "Lake")))) %>%
...
gg_tp_fe <- ggplot(data = gg_tp) + 
  ...
  expand_limits(y = -1.8) +
  geom_rect(ymin = -0.6, ymax = -0.6 - 2, xmin = 0, xmax = nrow(gg_tp) + 1, 
              fill = "transparent") +
  geom_text(aes(x = pretty, label = pretty, color = signif), 
            y = -0.65, vjust = 0.5, hjust = "right") + 
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "red")) +
    geom_hline(yintercept = 0) +
  scale_y_continuous(breaks = seq(from = -0.6, to  = 0.4, by = 0.2), 
...
gg_tp_re <- ggplot(data = fit_tp$re) +
  ...
  ggtitle("TP ~ Ag slope") + xlab("Zone ID") + ylab("") +
  theme(legend.position = 0)

gg_tp_re
```

```{r tn_fe, out.width="3.25in", out.height="4in"}
# lapply(re_brms, function(x) x$formula) 
...
gg_tn_fe <- 
  ...
  geom_pointrange(aes(x = pretty, y = `50%`, 
                      ymin = `5%`, ymax = `95%`, color = signif)) + 
  expand_limits(y = c(-0.6, 1.6)) +
  geom_rect(ymin = 0.44, ymax = 0.44 + 2, xmin = 0, xmax = nrow(gg_tn) + 1, 
              fill = "transparent") +
  geom_text(aes(x = pretty, label = pretty, color = signif), 
            y = 0.44, vjust = 0.5, hjust = "left") + 
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "red")) +
    geom_hline(yintercept = 0) +
  scale_y_continuous(breaks = seq(from = -0.6, to  = 0.4, by = 0.2), 
                     labels = scales::number_format(accuracy = 0.1)) +
  ...
  theme(axis.text.y = element_blank(), 
        axis.ticks.y = element_blank(), 
        axis.title.y = element_blank()) + 
    facet_wrap(~category, strip.position = "right", ncol = 1, scales = "free_y",
               as.table = FALSE) +
    theme(panel.spacing = unit(0, "lines"), 
          strip.placement = "outside", 
          strip.background = element_blank(), 
...
gp <- ggplotGrob(gg_tn_fe)
facet.columns <- gp$layout$t[grepl("panel", gp$layout$name)]
x.var         <- sapply(ggplot_build(gg_tn_fe)$layout$panel_scales_x,
                function(l) length(l$range$range))
gp$heights[facet.columns] <- gp$heights[facet.columns] * rev(x.var)
gg_tn_fe <- ggplotify::as.ggplot(gp)
gg_tn_fe
```
...
gg_hu4 <- 
  ggplot() +
    ...
    geom_sf(data = states, fill = "gray", color = "black", size = 0.3) +
    geom_sf(data = hu4, fill = "white", color = "black", size = 0.4) +
...
gg_tn_re <- ggplot(data = fit_tn$re) +
  ...
  cowplot::theme_cowplot() +
  theme(rect = element_rect(fill = "transparent")) +
  ggtitle("TN ~ corn slope") + xlab("Zone ID") + ylab("") +
  theme(legend.position = 0)
...
```{r fe}
cowplot::plot_grid(gg_tp_fe + theme(axis.title.x = element_blank()), 
                   gg_tn_fe + theme(axis.text.y = element_blank(), 
