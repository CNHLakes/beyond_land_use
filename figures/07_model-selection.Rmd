...
```{r 07_model_selection, fig.height=3, echo=FALSE, fig.width=6, eval=TRUE}
print("07_model_selection")
# setwd("figures")
...
fit_tn         <- re_brms[[5]] # 5 is best tn model with re effects
...
gg_corn_re <- ggplot(data = fit_tn$re) +
...
fit_tp <- re_brms[[3]] # 3 is best tp model with re effects
...
spread_draws(fit_tn, 
                     ...
                     b_buffervcultivatedvcrops) %>%
  tidyr::gather(key = "key", value = "value", -.chain, -.iteration, -.draw) %>%
  mutate(key = factor(key, levels = c("b_maxdepth", 
                     "b_soilvorgvcarbon", 
                     "b_nitrogenvfertilizervuse", 
                     "b_buffervcultivatedvcrops"))) %>%
  ggplot(aes(x = value)) +
