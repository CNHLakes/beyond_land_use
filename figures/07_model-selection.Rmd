...
gg_tp <- spread_draws(fit_tp, `b_.*`, regex = TRUE) %>%
  ...
  mutate(category = factor(category, 
                           levels = rev(c("Nutrient inputs", "Nutrient transport", 
                                      "Land-use cover", "Buffer configuration", 
                                      "Lake")))) %>%
...
re_global$response <- c(rep("tp", 5), rep("tn", 5))
re_global <- dplyr::filter(re_global, term != "rowvcropvpct")

# loo_compare(loo(re_brms[[1]]), loo(re_brms[[2]]), loo(re_brms[[3]]),
#             loo(re_brms[[4]]), loo(re_brms[[5]]))
# loo_compare(loo(re_brms[[6]]), loo(re_brms[[7]]), loo(re_brms[[8]]), 
#             loo(re_brms[[9]]), loo(re_brms[[10]]))
...
cowplot::plot_grid(gg_tp, gg_tn, rel_widths = c(1, 0.8))
```
