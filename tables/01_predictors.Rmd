...
dt <- readRDS("../data/dt.rds") %>%
  dplyr::select(tp:clay_pct, buffer_cultivated_crops, buffer_natural,
                -matches("zoneid"), -matches("pct"), -matches("input"), cdl_vars) %>%
...
dt <- mutate(dt, pretty = case_when(
  ...
  TRUE ~ pretty), 
  IQR = paste0(q25, " - ", q75))
dt <- dplyr::select(dt, Variable = pretty, 
                    Median = q50, Q25 = q25, Q75 = q75)
